...
class Home extends CI_Controller
{
    ...
    function profile($para1="",$para2="",$para3="")
    {
        ...
        else if ($para1 == 'sticky_add')
        {
            ...
            $food_paring = array('food_name1'=>$this->input->post('food_name1'),'food_name2'=>$this->input->post('food_name2'),'food_name3'=>$this->input->post('food_name3'),'food_name4'=>$this->input->post('food_name4'));

            $data['food_paring']  = json_encode($food_paring);
            $data['note']  = $this->input->post('note');
            $data['user_id']      = $this->session->userdata('user_id');

            $this->db->insert('sticky',$data);
        ...
        elseif($para1=="sticky")
        {
            $page['sticky_data'] = $this->db->get_where('sticky',array('user_id'=>$this->session->userdata('user_id')))->result_array();
            $this->load->view('front/user/sticky',$page);
        }
        elseif($para1 == 'withdrawal_add')
        {
        ...
        {
            $page_data_wishlist['query_wishlist']= array();
            $id = $this->session->userdata('user_id');
            $idss = json_decode($this->db->get_where('user',array('user_id'=>$id))->row()->wishlist,true);
            if(count($idss) > 0)
            {
                $this->db->where_in('product_id', $idss);
                $page_data_wishlist['query_wishlist'] = $this->db->get('product', $config['per_page'], $para2)->result_array();  
            }
            $this->load->view('front/user/wishlist',$page_data_wishlist);
        ...
        elseif($para1=="order_tracing")
        {
            ...
            if($sale_data->num_rows() >= 1){
                $page_data['sale_code'] = $this->input->post('sale_code');
    ...
    function ticket_listed($para2='')
    {
        ...
        $config['full_tag_open']  = '<ul class=" pagination-style-2 pagination-sm">';
        $config['full_tag_close'] = '</ul>';
    ...
    function order_listed($para2='')
    {
        ...
        $config['first_link']      = '&laquo;';
        $config['first_tag_open']  = '<li><a rel="grow" class="" onClick="' . $function . '">';
        $config['first_tag_close'] = '</a></li>';
        ...
        $config['last_link']      = '&raquo;';
        $config['last_tag_open']  = '<li><a rel="grow" class="" onClick="' . $function . '">';
        $config['last_tag_close'] = '</a></li>';
        
        $function                 = "order_listed('" . ($para2 - $config['per_page']) . "')";
        $config['prev_tag_open']  = '<li><a rel="grow" class="" onClick="' . $function . '">';
        $config['prev_tag_close'] = '</a></li>';
        
        $function                 = "order_listed('" . ($para2 + $config['per_page']) . "')";
        $config['next_link']      = '&rsaquo;';
        $config['next_tag_open']  = '<li><a rel="grow" class="" onClick="' . $function . '">';
        $config['next_tag_close'] = '</a></li>';
        
        $config['full_tag_open']  = '<ul>';
        $config['full_tag_close'] = '</ul>';
        
        $config['cur_tag_open']  = '<li class="active"><a rel="grow"  class="active">';
        $config['cur_tag_close'] = '</a></li>';
    ...
    function wish_listed($para2='')
    {
        ...
        $config['num_tag_open']  = '<li><a rel="grow" class="" onClick="' . $function . '">';
        $config['num_tag_close'] = '</a></li>';
        $this->ajax_pagination->initialize($config);
        $ids = json_decode($this->db->get_where('user',array('user_id'=>$id))->row()->wishlist,true);
        $this->db->where_in('product_id', $ids);
    ...
    function wallet_listed($para2='')
    {
        ...
        $transaction = $this->db->get_where('transaction',array('user_id'=>$id,'status'=>'paid'))->result_array();

        $refer = $this->db->get_where('refer',array('user_id'=>$id,'status'=>'Joined'))->result_array();
       
        $wallet = $this->db->get_where('wallet_load',array('user'=>$id,'status'=>'paid'))->result_array();
       
        foreach ($transaction as $row1) 
        {
            $arr[] = array('created_date'=>strtotime($row1['date']),'detail'=>$row1);
        }
        foreach ($refer as $row2) 
        {
            $check_id = $this->db->get_where('user',array('email'=>$row2['refer_email']))->row()->user_id;
            $order_buyer = $this->db->get_where('sale',array('buyer'=>$check_id))->row()->buyer;
            if($order_buyer)
            {     
                $arr[] = array('created_date'=>strtotime($row2['date']),'detail'=>$row2);
            }
        }

        foreach ($wallet as $row3) 
        {
    ...
    function category($para1 = "", $para2 = "", $min = "", $max = "", $text ='')
    {
        ...
        $page_data['cur_brand']        = $brand;
        $page_data['cur_category']     = $para1;
        $page_data['text']             = str_replace('_', ' ', $text);
    ...
    function affiliate()
    {
        $page_data['page_name']        = "others/affiliate";
        $page_data['asset_page']       = "all_category";
        $page_data['page_title']       = translate('omgee_affiliate_program');
        $this->load->view('front/index', $page_data); 
    }
    function events($para1="",$para2="")
    {
        $page_data['page_name']        = "others/events";
        $page_data['asset_page']       = "all_category";
        $page_data['page_title']       = translate('events');

        $page_data['events_details']=$this->db->get_where('events', array('events_id' => $para1,'status' => 'approved'))->row();
        $this->load->view('front/index', $page_data);
    }

    function refer($para1="",$para2="")
    {
        $page_data['page_name']        = "others/refer";
        $page_data['asset_page']       = "all_category";
        $page_data['page_title']       = translate('refer');
        $page_data['refers']=$this->db->get('refer')->result_array();
       
        $this->load->view('front/index', $page_data);
    }

    function event_remaining_time()
    {
        $time_arr = array();
        $events_id = $this->input->post('events_id');
        $events_details =$this->db->get_where('events', array('events_id' => $events_id,'status' => 'approved'))->row();  
        $current_date = date('Y-m-d');
        $current_datetime =  date("Y-m-d H:i:s", time());
        $events_start_datetime = date("$events_details->date $events_details->start_time");
        $events_end_datetime = date("$events_details->date $events_details->end_time");

        $remaining_time = strtotime($current_datetime) - strtotime($events_start_datetime);
        $time_arr['events_remaining_time'] = $remaining_time;

        $time_arr['end_of_event_time'] = ((strtotime($events_end_datetime) - strtotime($events_start_datetime)) - $remaining_time)+1;        
    ...
    function sticky_search()
    {
        ...
        $product_details =$this->db->get_where('product', array('product_id' =>'525','title' => 'skol'))->row();  
        
        $search_arr['food_name1'] = $product_details->food_name1;        
        $search_arr['food_name2'] = $product_details->food_name2;        
        $search_arr['food_name3'] = $product_details->food_name3;        
        $search_arr['food_name4'] = $product_details->food_name4;        

        echo json_encode($search_arr);
    }
    function how_omgee_work()
    {
        $page_data['page_name']        = "others/how_omgee_work";
        $page_data['asset_page']       = "all_category";
        $page_data['page_title']       = translate('how_omgee_work');
        $this->load->view('front/index', $page_data); 
    }
    function about_us()
    {
    ...
    function text_search()
    {
        if ($this->crud_model->get_settings_value('general_settings','vendor_system') !== 'ok') 
        {
            ...
            $search = trim($this->input->post('query'));
            if($search!=ctype_space($search))
            {
                $s = preg_replace("/\s+/", "_",$search);
                $category = $this->input->post('category');
                $this->session->set_flashdata('query',$search);
                redirect(base_url() . 'home/category/'.$category.'/0-0/0/0/'.$s, 'refresh');
            }
            else
            {
                redirect(base_url());
        ...
        else
        {
            ...
            $text = urlencode($this->input->post('query'));
            $arr = explode('+',trim($text));
            $search = $arr[0];
            redirect(base_url() . 'home/category/'.$category.'0/0-0/0/0/'.$search, 'refresh');
        ...
        { 
            ...
            foreach($cashback_product as $key => $value) 
            {
                ...
                if(strtotime($value['till']) > strtotime($current_date))
                {
                    ...
                    {
                        if($already_add_product_ar->discount_type == 'percent')
                        {
                            $already_add_product_arr[] = array('productid'=>$productids,'discount_value'=>$already_add_product_ar->discount_value);
                        }    
                    }
                }
            ...
            if (isset($range)) {
                $p = explode(';', $range);
                $this->db->where('bundle_sale1 >=', $p[0]);
                $this->db->where('bundle_sale1 <=', $p[1]);
            }
            ...
            $config['cur_tag_open']  = '<li><a rel="grow"  class="active">';
            $config['cur_tag_close'] = '</a></li>';
            ...
            if($sort== 'price_low')
            {
                $this->db->order_by('bundle_sale1', 'asc');
            }
            if($sort== 'price_high')
            {
                $this->db->order_by('bundle_sale1', 'desc');
            }
            if($sort== 'a_to_z')
            {
            ...
            if($sort== 'product_year')
            {
                $this->db->order_by('product_year', 'desc');
            }
            if($sort== 'product_abv')
            {
                $this->db->order_by('product_abv', 'desc');
            }
            if($sort== 'discount')
            {
                $this->db->order_by('bundle_discount1', 'desc');
            }
            if($sort== 'discount')
            {
                $this->db->order_by('bundle_discount1', 'desc');
             ...
             * 
             */
            if($sort == "limited_release")
                $this->db->where('limited_release',"Yes");
            
            
            $page_data['sort_by'] = $sort;

            if($sort== 'cashback')
            {
                $all_products_arr = $this->db->get('product', $config['per_page'], $para2)->result_array();
                $with_cashback_product = array();
                
                foreach ($all_products_arr as $key => $value) 
                {
                    $productKey = $this->searchArrayKeyVal("productid",$value['product_id'],$already_add_product_arr);
                    
                    if($productKey!==false) 
                    {
                        $coupon_price = array('product_id'=>$productKey,'cashback_price'=>$already_add_product_arr[$productKey]['discount_value']);
                        $with_cashback_product[] = array_merge($coupon_price,$value);
                    }
                }
                $sort_by_cashback = $this->array_msort($with_cashback_product, array('cashback_price'=>SORT_DESC));
                $page_data['all_products'] = $sort_by_cashback;
            }
            else
            {
    ...
    function product_view($para1 = "",$para2 = "")
    {
        ...
        $category = $page_data['product_details'][0]['category'];
        $category_detail = $this->db->get_where('category',array('category_id' => $category))->row();
        $page_data['category_name'] =  $category_detail->category_name;
        $this->load->view('front/index', $page_data);
    }
   ...
   function vendor_logup($para1 = "", $para2 = "")
    ...
    function registration($para1 = "", $para2 = "")
    {
        ...
        if ($para1 == "add_info") {
            $msg = '';
            $this->form_validation->set_rules('titlename', 'Titlename', 'required');
            ...
            $this->form_validation->set_rules('country', 'Country', 'required');
            $this->form_validation->set_rules('comfirm', 'Terms & Conditions', 'required', array('required' => translate('you_must_agree_with_terms_&_conditions')));
                ...
                if($safe == 'yes'){
                    if($this->crud_model->get_settings_value('general_settings','captcha_status','value') == 'ok'){
                        ...
                        if ($response['success']) {
                            ...
                            $data['unique_refer_key']      = base64_decode($this->input->post('unique_refer_key'));
                            $data['email']         = $this->input->post('email');
                            $data['phone']         = $this->input->post('phone');
                            $data['comfirm']       = $this->input->post('comfirm');
                            ...
                            //refer_insert
                                ...
                                $refer_email_check = $this->db->get_where('refer',array('refer_email'=>$data['email']))->row()->refer_email;
                                $user_refer_id = $this->db->get_where('refer',array('refer_email'=>$user_refer_id_check))->row()->user_id;
                                 
                                if($user_refer_id_check)
                                {
                                    if( ($refer_email_check == $data['email']))
                                    {
                                        $refer_data = array();
                                        $refer_data['refer_user_id'] = $data['unique_refer_key'];
                                        ...
                                        $refer_data['user_id'] = $user_refer_id_check;
                                ...
                                $msg = 'done';
                                if($this->email_model->account_opening('user', $data['email'], $password, $data['verification_key']) == false){
                                    $msg = 'done_but_not_sent';
                                }else{
                                    $msg = 'done_and_sent';
                                }
            ...
            $data['dob']         = $this->input->post('dob');
            $data['email']       = $this->input->post('email');
            $remove_last_image = $this->input->post('last_profile_image');
            ...
            { 
                ...
                if(in_array($ext,$extension)) 
                {
                    ...
                    move_uploaded_file($file_tmp,"uploads/user_image/".$newFileName);
                    if($remove_last_image)
                    {
                        unlink("uploads/user_image/".$remove_last_image);
                    }    
                }
                else
                {
                    $data['profile_image'] = $remove_last_image;
                }
            }
    ...
    function cart($para1 = '', $para2 = '', $para3 = '', $para4 = '')
    {
        ...
        if ($para1 == "add") 
        {
            ...
            $variationqty = $this->input->post('variationqty');
            $option = array('productid'=>$para2,'variationid'=>$variationid,'variationqty'=>$variationqty,'coupon_price'=>$coupon_price);
            ...
            $product_details = $this->db->get_where('product',array('product_id'=>$para2))->row();
            ...
            if($this->session->userdata('currency') == '10')
            {
                ...
                {
                    $rrp = $product_details->sale_price_HK*$qty;
                }
                else
                {
                    $rrp = $product_details->sale_price_AU*$qty;
            ...
            if($this->session->userdata('currency') == '13')
            {
                ...
                {
                    $rrp = $product_details->sale_price_JP*$qty;
                }
                else
                {
                    $rrp = $product_details->sale_price_AU*$qty;
            ...
            if($this->session->userdata('currency') == '22')
            {
                ...
                {
                    $rrp = $product_details->sale_price_SG*$qty;
                }
                else
                {
                    $rrp = $product_details->sale_price_AU*$qty;
            ...
            $discount = ($product_details->discount) ? ($product_details->discount/100) : 0;
            ...
            if($num_of_imgs->num_of_imgs !=NULL)
            {
                $num_of_img = explode(",", $num_of_imgs->num_of_imgs); 
            ...
            if(!$this->crud_model->is_added_to_cart($para2,'option',$variationid)) 
            {
        ...
        if ($para1 == "userwallet") 
        {
            ...
            {    
                $optiondata = json_decode($row['option'],true);
                $bundle_condition = "discount";
                $discount = $this->db->get_where('product',array('product_id'=>$optiondata['productid']))->row()->$bundle_condition;

                $for_producttotal = ($row['price']/$optiondata['variationqty'])*$row['qty'];      
                $sub_total+= $for_producttotal - (($discount/100)*$for_producttotal); 
            }
            ...
            if($walletamount > 0)
            {
                ...
                $total = (($sub_total+$shipping) - $walletamount);
            }
            else
            {
            ...
            if($total > 0)
            {
               echo json_encode(currency($total));
               $this->session->set_userdata('grand_total',$total);
            }
            else
            {
                echo json_encode(currency().'0');
                $this->session->set_userdata('grand_total',0);
            }           
        }
        ...
        {            
            ...
            foreach ($carted as $items) 
            {
                ...
                {
                    $options_pro_id = json_decode($items['option']);
                    $product = $options_pro_id->productid;
                }
            }
            
            $current_quantity = 72;/*$this->crud_model->get_type_name_by_id('product', $product, 'current_stock');*/
            $msg              = 'not_limit';
            foreach ($carted as $items) {
                if ($items['rowid'] == $para2) {
            ...
            foreach($carted as $items) 
            {
                if($items['rowid'] == $para2) 
                {
                    ...
                    $cart_options = json_decode($items['option'], true);

                    if($cart_options['variationqty'] == 6) {
                        $rrp = $rrp*95/100;
                        $orp = $orp*95/100;
                    }

                    if($cart_options['variationqty'] == 12) {
                        $rrp = $rrp*90/100;
                        $orp = $orp*90/100;
                    }
            ...
            if ($para2 == 'full') 
            {
                ...
                {    
                    ...
                    if($productKey > 0 ) 
                    {
                        ...
                        {
                            $cashback_price += (($discount_value)*$items['qty']);
                        }
                    }    
                ...
                $total_cashback_discount = "$".$total_cashback_discount;
                ...
                if($this->input->get('insurance') != "AU"){
                    if($sub_total > 1000)
                        $insurance_amount = number_format(($sub_total*2)/100,2);
                    else 
                        $insurance_amount = 20;
                }
                if($this->input->get('add_insurance')){
                    $grand = $sub_total + $shipping + $insurance_amount ;
                }
                ...
                if($this->session->userdata('currency') == '2')
                {
                    ...
                    $insurance_currency = currency($insurance_amount);
                }
                if($this->session->userdata('currency') == '10')
                {
                    if($product_details->sale_price_HK > 0)
                ...
                echo $with_currency_producttotal. '-' . $ship . '-' .$with_currency_tax. '-' . $with_currency_grand . '-' . $count . '-' . $with_currency_total_saving . '-' . $total_cashback_discount . '-' . $with_currency_sub_total . '-' . $coupon_price . '-' . $with_currency_total_promocode . '-'.$insurance_currency;
    ...
    function promocode_check()
    {
        ...
        if (count($carted) > 0) 
        {
            $promocode_detail = $this->db->get_where('promocode',array('code'=>$para1))->row();
            if(($promocode_detail==null)||($para1==null))
            {
                echo 'nope';
            }
            $promocode_spec = json_decode($promocode_detail->spec);
            $pr_discount_price = ($promocode_spec->discount_value) ? $promocode_spec->discount_value : 0 ;
            $events_details = $this->db->get_where('events',array('promocode_id'=>$promocode_detail->promocode_id))->row();
            $events_products = explode(",",$events_details->promocode_products);
            $spec = json_decode($promocode_detail->spec,true);
            $promocode_id = $promocode_detail->promocode_id;
            ...
            if(($from < time())&&($till > time()))
            {
                ...
                foreach ($carted as $items) 
                {
                    $optiondata = json_decode($items['option'],true);
                    if(in_array($optiondata['productid'],$events_products))
                    {
                        $discount = $this->db->get_where('product',array('product_id'=>$optiondata['productid']))->row()->bundle_discount1;
                    
                        $promocode_cal_discount_price = $items['price']*($pr_discount_price/100);

                        $option = array('productid'=>$optiondata['productid'],'variationid'=>$optiondata['variationid'],'variationqty'=>$optiondata['variationqty'],'coupon_price' => 0,'promocode_discount_price'=>$pr_discount_price,'promocode_cal_discount_price'=>$promocode_cal_discount_price);

                        $this->session->set_userdata('promocode',$para1);
                    }
                    else
                    {
                        $option = array('productid'=>$optiondata['productid'],'variationid'=>$optiondata['variationid'],'variationqty'=>$optiondata['variationqty'],'coupon_price' => ($base_price-$discount),'promocode_discount_price'=>0,'promocode_cal_discount_price'=>0);
                    }
                    $data = array('rowid' => $items['rowid'],'option'=>json_encode($option));
                    $this->cart->update($data);
                }
            }
        }
                    ...
                    if(($from < time())&&($till > time()))
                    {
                        ...
                        if($set_type !== 'total_amount'){
                            ...
                            foreach ($carted as $items) 
                            {
                                ...
                                if(in_array($optiondata['productid'], $dis_pro)) 
                                {
                                    $bundle_condition = "bundle_sale".$optiondata['variationid'];
                                    $base_price = $this->db->get_where('product',array('product_id'=>$optiondata['productid']))->row()->$bundle_condition;
                                    if($type == 'percent')
                                    {
                                    ...
                                    $option = array('productid'=>$optiondata['productid'],'variationid'=>$optiondata['variationid'],'variationqty'=>$optiondata['variationqty'],'coupon_price' => ($base_price-$discount));
                                    $data = array(
                                        'rowid' => $items['rowid'],
                                        'coupon' => $coupon_id,
                                        'option'=>json_encode($option)
    ...
    function cart_finish($para1 = "", $para2 = "")
    {
        ...
        if($this->input->post('insurance'))
            $insurance_amount = str_replace("$","",$this->input->post('insurance'));
        $grand_total     = $total + $shipping + $insurance_amount;
            ...
            if ($para1 == 'go') {
                if(isset($_POST['stripeToken'])) 
                {
                    ...
                    if($charge->paid == true)
                    {
                        ...
                        {
                            redirect(base_url() . 'home/thankyou/' . $data['guest_id'], 'refresh');
                        }
                    }
                    else
                    {
    ...
    /* FUNCTION: thankyou showing*/
    function thankyou($para1 = "", $para2 = "")
    {
        if ($this->session->userdata('user_login') != "yes" || $this->crud_model->get_type_name_by_id('sale', $para1, 'buyer') !=  $this->session->userdata('user_id'))
        {
            redirect(base_url(), 'refresh');
        ...
        $page_data['sale_id']    = $para1;
        $page_data['asset_page']    = "invoice";
        $page_data['page_name']  = "shopping_cart/thankyou";
        $page_data['page_title'] = translate('thankyou');
        $this->load->view('front/index', $page_data);
    }
    
    function verification()
    {
        if($_GET['verifykey'] == null)
        {
            redirect(base_url(), 'refresh');
        }
        $data['status'] = 'complete';
        $this->db->where('verification_key',base64_decode($_GET['verifykey']));
        $this->db->update('user', $data);

        $page_data['verifykey']  = $_GET['verifykey'];
        $page_data['asset_page'] = "invoice";
        $page_data['page_name']  = "others/verify";
        $page_data['page_title'] = translate('Email Verification');
        $this->load->view('front/index', $page_data);
    }
    ...
    function others($para1 = "", $para2 = "", $para3 = "", $para4 = "")
    {
        if ($para1 == "get_sub_by_cat") {
            ...
            $this->db->insert('events_registration',$data);
            
            redirect(base_url('home/events/'.$events_id), 'refresh');
        }  else if ($para1 == 'do_refer') {

            $data['refer_email']     = $this->input->post('email');
            $data['user_id']     = $this->session->userdata('user_id');
    ...
    /**
     * 
     * testing email added by niraj lal rahi
     * 
     */

    public function testEmail(){
        $from = $this->db->get_where('general_settings',array('type' => 'smtp_user'))->row()->value;
        $to = $this->input->get('to') ? $this->input->get('to') : 'krniraj007@gmail.com';
        echo $this->email_model->do_email(
            $from , 
            $from_name = 'Niraj', $to , $sub ='test', $msg ='test');
    ...
    function sticky_status_change()
    {
        $sticky_id = $this->input->post('sticky_id');
        $status = $this->input->post('status');

        $result = $this->db->get_where('sticky',array('sticky_id'=>$sticky_id))->row()->status;
        if($result)
        {
            $this->db->where('sticky_id', $sticky_id);

            if($status == 'private')
            {
                $data['status'] = 'public';
                $this->db->update('sticky', $data);
                echo $this->db->last_query();
                echo "success";
            }
            if($status == 'public')
    ...
    {   
        ...
        if($select_val)
        {
            $result = $this->db->get_where('product',array('product_id'=>$select_val))->row();          
            $output = array('food_name1'=>ucwords($result->food_name1),'food_name2'=>ucwords($result->food_name2),'food_name3'=>ucwords($result->food_name3),'food_name4'=>ucwords($result->food_name4));

            echo json_encode($output);
        }
    }
    function ishippingCal()
    {
        $carted = $this->cart->contents();
        $alcohol_list = array();
        $volume_list = array();
        ...
        foreach ($carted as $row) 
        {
            ...
            $result = $this->db->get_where('product',array('product_id'=>$options->productid))->row();
            
            $alcohol_list[] = (string) $result->product_abv;
            $volume_list[]  = (string) $options->variationqty;
            $weight_list[]  = $options->variationid;
            $price_list[]   = $row['price'];
            $quantity_list[]= $row['qty'];

            $category = $this->db->get_where('category',array('category_id'=>$result->category))->row();
            $category_product_list[] = (string) strtoupper($category->category_name);
        }
        ...
        $Streetlines = array($this->input->post('address1'),$this->input->post('address2'));
        $fullname = $this->input->post('first_name')." ".$this->input->post('last_name');
        $data = array(
            "api_key" =>"p6RertCvahmbQm28Byky",
            "type" => $this->input->post('country') == "AU" ? "domestic" : "overseas",
            ...
            "recipient_details" => array(
                ...
                "Address" => array(
                    ...
                    "StateOrProvinceCode" => $this->input->post('country'),
        ...
        //         "Address" => array(
        //             "Streetlines" => "3 Chome-5-4 Shinjuku, Shinjuku City, Tokyo 160-0022, Japan",
        //             "City" =>  "Tokyo",
        ...
        $shipping_price_arr = json_decode($result);
        if($shipping_price_arr->postage_result->status == 'success')
        {
            $shipping_price = $shipping_price_arr->postage_result->data->total_cost;
        }
        else
        {
            $shipping_price = 0;
        ...
        $this->session->set_userdata('ishipping_total_price',$shipping_price);
    }
    function send_order_ishipping($sale_code,$posted)
    {
        $carted = $this->cart->contents();
        $alcohol_list = array();
        $volume_list = array();
        ...
        $Streetlines = array($posted['address1']);
        $fullname = $posted['first_name']." ".$posted['last_name'];
        $data = array(
                        "api_key" =>"p6RertCvahmbQm28Byky",
                        "type" => "overseas",
                        "alcohol_list" => $alcohol_list,
                        ...
                        "recipient_details" => array(
                            "PersonName" => $fullname,
                            "CompanyName" => "Google",
                            "PhoneNumber" => $posted['phone'],
                            "Email" => $posted['email'],
                            "Address" => array(
                                "Streetlines" => $Streetlines,
                                "City" => $posted['city'],
                                "StateOrProvinceCode" => $posted['state_or_province_code'],
                                "PostalCode" => $posted['postal_code'],
                                "CountryCode" => $posted['country'],
                                "Residental" => false
                            )
                        )
                    );
