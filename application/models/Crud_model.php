...
class Crud_model extends CI_Model
{
    ...
    function file_up($name, $type, $id, $multi = '', $no_thumb = '', $ext = '.jpg', $no_of_img = '')
    {
        if ($multi == '') {
            ...
            $ext = '.' . $extension;
            if ($no_of_img > 0) {
                move_uploaded_file($_FILES[$name]['tmp_name'], 'uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $no_of_img . $ext);
            } else {
    ...
    function file_up_top($name, $type, $id, $multi = '', $no_thumb = '', $ext = '.jpg', $no_of_img = '')
    {
        if ($multi == '') {
            $filename   = $_FILES[$name]['name'];
            $extension  = pathinfo($filename, PATHINFO_EXTENSION);
    ...
    function file_view($type, $id, $width = '100', $height = '100', $thumb = 'no', $src = 'no', $multi = '', $multi_num = '', $ext = '.jpg')
    {
        if ($multi == '') {
            ...
            // if ($type == "path" && file_exists('uploads/'.$type)) { 

            //     $srcl = base_url() . 'uploads/' . $type;
            ...
            //elseif (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . $ext)) { }

            if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '.png')) {
                $ext = '.png';
            } elseif (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '.gif')) {
                $ext = '.gif';
            } elseif (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '.jpeg')) {
                $ext = '.jpeg';
            } else {
                $ext = '.jpg';
            }
            ...
            while ($p < $num) {
                $i++;
                if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $i . '.png')) {
                    $ext = '.png';
                } elseif (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $i . '.gif')) {
                    $ext = '.gif';
                } elseif (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $i . '.jpeg')) {
                    $ext = '.jpeg';
                } else {
                    $ext = '.jpg';
                }
            ...
            } elseif ($num != "" &&  filter_var($num, FILTER_VALIDATE_INT) === false) {
                return base_url() . 'uploads/' . $type . '_image/' . $num;
            } else {
    ...
    function file_dlt($type, $id, $ext = '.jpg', $multi = '', $m_sin = '')
    {
        if ($multi == '') {
            ...
            if ($m_sin == '') {
                ...
                $p = 0;
                while ($p < $num) {
                    echo 'uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $i . '.png';
                    if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $i . '.jpg')) {
                        $ext = '.jpg';
                    }
                    if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $i . '.png')) {
                        $ext = '.png';
                    }
                    if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $i . '.jpeg')) {
                        $ext = '.jpeg';
                    }
    ...
    function product_image_dlt($type, $id, $ext = '.jpg', $multi = '', $m_sin = '', $no_of_img)
    {
        if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $no_of_img . '.png')) {
            $ext = '.png';
            unlink('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $no_of_img . $ext);
        } elseif (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $no_of_img . '.gif')) {
            $ext = '.gif';
            unlink('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $no_of_img . $ext);
        } elseif (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $no_of_img . '.jpeg')) {
            $ext = '.jpeg';
            unlink('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $no_of_img . $ext);
        } else {
            $ext = '.jpg';
    ...
    function select_html_order($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '', $condition_type = 'single', $is_none = '', $order = '')
    {
        $return = '';
        $other  = '';
        $multi  = 'no';
    ...
    function select_html_sub_order($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '', $condition_type = 'single', $is_none = '', $order = '')
    {
        $return = '';
        $other  = '';
        $multi  = 'no';
        ...
        if (!is_array($from)) {
            if ($condition == '') {
                ...
                if ($condition_type == 'single') {
                    // die(123);
                    if (!empty($order))
                        $this->db->order_by('sub_category_name', $order);
                    $all = $this->db->get_where($from, array(
                        $condition => $c_match
                    ))->result_array();
                } else if ($condition_type == 'multi') {
    ...
    function decrease_quantity($product_id, $quantity, $sale_id = '')
    {
        ...
        $sale_count = $this->crud_model->get_type_name_by_id('product', $product_id, 'sale_count');
        $data1['current_stock'] = $prev_quantity - $quantity;
        $data1['sale_count'] = $sale_count + $quantity;
    ...
    function product_by($product_id, $with_link = '')
    {
        ...
        if ($added_by['type'] == 'admin') {
            ...
            $name = $this->db->get_where('vendor', array('vendor_id' => $added_by['id']))->row()->name;
            if ($with_link == '') {
                return $name;
            } else if ($with_link == 'with_link') {
    ...
    function is_sale_of_vendor($sale_id, $vendor_id)
    {
        ...
        foreach ($product_details as $row) {
            ...
            if ($this->is_added_by('product', $optiondata['productid'], $vendor_id)) {

                $return[] = $optiondata['productid'];
            }
        }
        if (empty($return)) {
            return false;
    ...
    function is_wished($product_id)
    {
        if ($this->session->userdata('user_login') == 'yes') {
            ...
            //$wished = array('0');
            if (empty($user_id)) {
                if ($this->get_type_name_by_id('vendor', $vendor_id, 'wishlist') !== '') {
                    $wished = json_decode($this->get_type_name_by_id('vendor', $vendor_id, 'wishlist'));
                } else {
                    $wished = array(
                        '0'
                    );
    ...
    function product_list_set($speciality, $limit, $id = '', $cat_ids = '')
    {
        ...
        if ($speciality == 'featured') {
            ...
            $this->db->where('current_stock >=', '1');
            $this->db->order_by('feature_date', 'DESC');
        }
        ...
        if (!empty($cat_ids)) {
            $this->db->where_in('category', $cat_ids);
        }
...
>
    ...
    */
    ...
    function lastOneWeekproduct()
    {
        ...
        $latest = $this->db->query("SELECT * FROM product WHERE `created_date` >= curdate() - INTERVAL DAYOFWEEK(curdate())+6 DAY AND `remove`=0  ORDER BY product_id DESC");
         
        return  $latest->result_array();

    }
    function product_img_upload($total_img, $productid = '', $last_products_images = '')
    {
        $images_arr = array();
        $extension = array("jpeg", "jpg", "png", "gif");
        if ($productid > 0) {
            if ($last_products_images) {
                $images_arr = explode(",", $last_products_images);
            }
            for ($img = 0; $img <= count($total_img['name']); $img++) {
                ...
                if (in_array($ext, $extension)) {
                    ...
                    $newFileName = trim($filename . time()) . "." . $ext;
